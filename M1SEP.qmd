...
En algèbre, on définit un espace où les vecteurs appartiennent et on utilise le plus souvent un espace euclidien. C'est un espace où est rattaché une distance qu'on appelle produit scalaire. 
### Espace euclidien
...
-   **Variance** : La variance mesure la dispersion des valeurs autour de la moyenne : $$
    ...
    $$ 
    ou en théorie $$Var(X) = E(X^2) - (E(X))^2 = E((x - E(X))^2)$$ 
...
-   **Covariance** : La covariance entre deux variables $X$ et $Y$ mesure la manière dont deux variables varient ensemble.
...
:::: callout-warning
Ne pas confondre l'indépendance et l'incompatibilité des évènements
:::
#### Variable aléatoire
...
La covariance que nous avons traité plus haut dans les statistiques bivariées mesure l'intensité de la relation linéaire entre deux variables aléatoires X et Y. Elle est définie comme suit : $$Cov(X, Y) = E(XY) - E(X) \cdot E(Y)$$
